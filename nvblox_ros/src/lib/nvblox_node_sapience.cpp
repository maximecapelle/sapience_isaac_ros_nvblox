...
#include "nvblox_ros/nvblox_node_sapience.hpp"
...
void NvbloxNode::initializeMultiMapper()
{
  ...
  multi_mapper_1 = std::make_shared<MultiMapper>(voxel_size_, mapping_type_, esdf_mode_, MemoryType::kDevice);
  multi_mapper_1->setMultiMapperParams(multi_mapper_params_);
  multi_mapper_2 = std::make_shared<MultiMapper>(voxel_size_, mapping_type_, esdf_mode_, MemoryType::kDevice);
  multi_mapper_2->setMultiMapperParams(multi_mapper_params_);
  multi_mapper_3 = std::make_shared<MultiMapper>(voxel_size_, mapping_type_, esdf_mode_, MemoryType::kDevice);
  multi_mapper_3->setMultiMapperParams(multi_mapper_params_);

  // Get the mapper parameters
  const std::string static_mapper_name_1 = "static_mapper_1";
  const std::string dynamic_mapper_name_1 = "dynamic_mapper_1";
  const std::string static_mapper_name_2 = "static_mapper_2";
  const std::string dynamic_mapper_name_2 = "dynamic_mapper_2";
  const std::string static_mapper_name_3 = "static_mapper_3";
  const std::string dynamic_mapper_name_3 = "dynamic_mapper_3";
  declareMapperParameters(static_mapper_name_1, this);
  declareMapperParameters(dynamic_mapper_name_1, this);
  declareMapperParameters(static_mapper_name_2, this);
  declareMapperParameters(dynamic_mapper_name_2, this);
  declareMapperParameters(static_mapper_name_3, this);
  declareMapperParameters(dynamic_mapper_name_3, this);
  MapperParams static_mapper_params_1 = getMapperParamsFromROS(static_mapper_name_1, this);
  MapperParams dynamic_mapper_params_1 = getMapperParamsFromROS(dynamic_mapper_name_1, this);
  MapperParams static_mapper_params_2 = getMapperParamsFromROS(static_mapper_name_2, this);
  MapperParams dynamic_mapper_params_2 = getMapperParamsFromROS(dynamic_mapper_name_2, this);
  MapperParams static_mapper_params_3 = getMapperParamsFromROS(static_mapper_name_3, this);
  MapperParams dynamic_mapper_params_3 = getMapperParamsFromROS(dynamic_mapper_name_3, this);

  // Set the mapper parameters
  multi_mapper_1->setMapperParams(static_mapper_params_1, dynamic_mapper_params_1);
  multi_mapper_2->setMapperParams(static_mapper_params_2, dynamic_mapper_params_2);
  multi_mapper_3->setMapperParams(static_mapper_params_3, dynamic_mapper_params_3);
  ...
  static_mapper_1 = multi_mapper_1.get()->unmasked_mapper();
  dynamic_mapper_1 = multi_mapper_1.get()->masked_mapper();
  static_mapper_2 = multi_mapper_2.get()->unmasked_mapper();
  dynamic_mapper_2 = multi_mapper_2.get()->masked_mapper();
  static_mapper_3 = multi_mapper_3.get()->unmasked_mapper();
  dynamic_mapper_3 = multi_mapper_3.get()->masked_mapper();
}
...
void NvbloxNode::advertiseTopics()
{
  ...
  // SAPIENCE ESDF
  static_esdf_pointcloud_publisher_1 = create_publisher<sensor_msgs::msg::PointCloud2>("~/sapience_esdf_1", 1);
  static_esdf_pointcloud_publisher_2 = create_publisher<sensor_msgs::msg::PointCloud2>("~/sapience_esdf_2", 1);
  static_esdf_pointcloud_publisher_3 = create_publisher<sensor_msgs::msg::PointCloud2>("~/sapience_esdf_3", 1);
  // DISTANCE SLICE
  static_map_slice_publisher_1 = create_publisher<nvblox_msgs::msg::DistanceMapSlice>("~/sapience_esdf_slice_1", 1);
  static_map_slice_publisher_2 = create_publisher<nvblox_msgs::msg::DistanceMapSlice>("~/sapience_esdf_slice_2", 1);
  static_map_slice_publisher_3 = create_publisher<nvblox_msgs::msg::DistanceMapSlice>("~/sapience_esdf_slice_3", 1);
  // Static occupancy
...
void NvbloxNode::processEsdf()
{
  ...
  multi_mapper_1->updateEsdf();
  esdf_integration_timer.Stop();

  timing::Timer esdf_output_timer("ros/esdf/output");

  sliceAndPublishEsdf(
    "static_1", static_mapper_1,
    static_esdf_pointcloud_publisher_1, static_map_slice_publisher_1);
  sliceAndPublishEsdf(
    "static_2", static_mapper_2,
    static_esdf_pointcloud_publisher_2, static_map_slice_publisher_2);
  sliceAndPublishEsdf(
    "static_3", static_mapper_3,
    static_esdf_pointcloud_publisher_3, static_map_slice_publisher_3);

  if (isUsingDynamicMapper(mapping_type_)) {
    sliceAndPublishEsdf(
      "dynamic", dynamic_mapper_1,
      dynamic_esdf_pointcloud_publisher_, dynamic_map_slice_publisher_);
    sliceAndPublishEsdf(
      "combined_dynamic", static_mapper_1,
      combined_esdf_pointcloud_publisher_, combined_map_slice_publisher_, dynamic_mapper_1.get());
  }
...
void NvbloxNode::processMesh()
{
  ...
  const std::vector<Index3D> mesh_updated_list = multi_mapper_1->updateMesh();
  ...
  if (new_subscriber_count > 0) {
    ...
    if (new_subscriber_count > mesh_subscriber_count_) {
      ...
      conversions::meshMessageFromMeshLayer(static_mapper_1->mesh_layer(), &mesh_msg);
      mesh_msg.clear = true;
      should_publish = true;
    } else {
      conversions::meshMessageFromMeshBlocks(
        static_mapper_1->mesh_layer(),
...
void NvbloxNode::decayDynamicOccupancy() {dynamic_mapper_1->decayOccupancy();}
  ...
  depth_camera_pair)
{
  ...
  multi_mapper_1->integrateDepth(depth_image_, T_L_C, camera, update_time_ms);
  ...
  const Transform & T_L_C)
{
  ...
  if (freespace_publisher_->get_subscription_count() > 0) {
    ...
    layer_converter_.pointcloudMsgFromLayerInAABB(
      static_mapper_1->freespace_layer(), aabb,
      &pointcloud_msg);
    pointcloud_msg.header.frame_id = global_frame_;
    pointcloud_msg.header.stamp = get_clock()->now();
  ...
  const std::string & camera_frame_id)
{
  ...
  if (dynamic_points_publisher_->get_subscription_count() > 0) {
    ...
    const Pointcloud & dynamic_points =
      multi_mapper_1->getLastDynamicPointcloud();
  ...
  if (dynamic_depth_frame_overlay_publisher_->get_subscription_count() > 0) {
    ...
    const ColorImage & dynamic_overlay =
      multi_mapper_1->getLastDynamicFrameMaskOverlay();
  ...
  if (dynamic_occupancy_publisher_->get_subscription_count() > 0) {
    sensor_msgs::msg::PointCloud2 pointcloud_msg;
    layer_converter_.pointcloudMsgFromLayer(dynamic_mapper_1->occupancy_layer(), &pointcloud_msg);
    pointcloud_msg.header.frame_id = global_frame_;
    pointcloud_msg.header.stamp = get_clock()->now();
  ...
  color_camera_pair)
{
  ...
  multi_mapper_1->integrateColor(color_image_, T_L_C, camera);
  ...
  const sensor_msgs::msg::PointCloud2::ConstSharedPtr & pointcloud_ptr)
{
  ...
  static_mapper_1->integrateLidarDepth(pointcloud_image_, T_L_C, lidar);
...
void NvbloxNode::publishOccupancyPointcloud()
{
  ...
  if (static_occupancy_publisher_->get_subscription_count() > 0) {
    sensor_msgs::msg::PointCloud2 pointcloud_msg;
    layer_converter_.pointcloudMsgFromLayer(static_mapper_1->occupancy_layer(), &pointcloud_msg);
    pointcloud_msg.header.frame_id = global_frame_;
    pointcloud_msg.header.stamp = get_clock()->now();
...
void NvbloxNode::clearMapOutsideOfRadiusOfLastKnownPose()
{
  if (map_clearing_radius_m_ > 0.0f) {
    ...
    {
      const std::vector<Index3D> blocks_cleared = static_mapper_1->clearOutsideRadius(
  ...
  std::shared_ptr<nvblox_msgs::srv::FilePath::Response> response)
{
  ...
  if (ends_with(request->file_path, ".ply")) {
    ...
    if (mapping_type_ == MappingType::kDynamic) {
      success &= io::outputVoxelLayerToPly(
        static_mapper_1->freespace_layer(),
  ...
  std::shared_ptr<nvblox_msgs::srv::FilePath::Response> response)
{
  ...
  response->success = static_mapper_1->saveLayerCake(filename);
  if (response->success) {
  ...
  std::shared_ptr<nvblox_msgs::srv::FilePath::Response> response)
{
  ...
  response->success = static_mapper_1->loadMap(filename);
  if (response->success) {
